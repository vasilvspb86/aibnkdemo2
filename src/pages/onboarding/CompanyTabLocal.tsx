...
export default function CompanyTabLocal() {
  const navigate = useNavigate();
  const { data, updateCompany, isLoading } = useLocalOnboarding();
  ...
  useEffect(() => {
    if (data.company) {
      setIssuingAuthority(data.company.issuing_authority || "");
      setTradeLicenseNumber(data.company.trade_license_number || "");
      setCompanyLegalName(data.company.company_legal_name || "");
      setLegalForm(data.company.legal_form || "");
      setRegisteredAddress(data.company.registered_address || "");
      setBusinessActivity(data.company.business_activity || "");
      setConfirmedByUser(data.company.confirmed_by_user || false);
      setPrefillSource(data.company.prefill_source);
      setLookupDone(!!data.company.company_legal_name);
    }
  }, [data.company]);
  ...
  const handleSave = () => {
    ...
    navigate("/onboarding-local/ownership");
  };
