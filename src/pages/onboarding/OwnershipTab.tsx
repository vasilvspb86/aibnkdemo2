...
export default function OwnershipTab() {
  ...
  const { data: persons, isLoading, upsertPerson, isUpdating } = useOnboardingPersons(caseId);
  ...
  // Roles are pre-selected for single owner
  const roles = ["owner", "director", "authorized_signatory"] as const;
  ...
  useEffect(() => {
    if (persons && persons.length > 0) {
      ...
      setDob(owner.dob ? format(new Date(owner.dob), "yyyy-MM-dd") : "");
      setNationality(owner.nationality || "");
      setEmail(owner.email || "");
      setPhone(owner.phone || "");
      setEmiratesIdNumber(owner.emirates_id_number || "");
      // Check if all required fields are filled
      if (owner.full_name && owner.dob && owner.nationality) {
        setConfirmed(true);
      }
    }
  }, [persons]);
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      navigate(`/onboarding/${caseId}/compliance`);
    } catch (error: any) {
